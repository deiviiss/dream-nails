...
export const CustomerSearch = ({ setSelectedCustomer }: CustomerSearchProps): JSX.Element => {
      ...
      {searchTerm !== '' && filteredCustomers.length > 0 && (
        <ul className='w-full bg-slate-50 p-1 border border-secondary border-t-0 rounded-lg'>
          {filteredCustomers.map((customer) => (
            <li key={customer.id} onClick={() => { handleResultClick(customer) }} className='px-1'>{customer.name}</li>
          ))}
        </ul>
      )}
    </div>
