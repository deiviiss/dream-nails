...
const CustomerEditPage: NextPage = () => {
  ...
  useEffect(() => {
    ...
    getCustomer()
  }, [params.id, setCustomer, getOneCustomer])
        ...
        <main className='flex flex-col items-center justify-center mx-auto gap-y-6 py-4 px-2 w-full max-w-lg mt-[150.5px]'>
          <header className='w-full flex flex-col justify-between'>
            <div className='flex justify-between'>
              ...
              <h1 className='text-base text-left font-medium'>Próxima cita</h1>
            </div>
            <div className='flex justify-between px-2'>
              <h1 className='text-xl text-left font-medium'>
                {customer?.name}
              </h1>
              <h1 className='text-xl text-left font-medium'>23-Oct-2023</h1>
            </div>
          </header>
          ...
          <div className='flex w-full justify-center my-5 gap-6'>
            <button
              onClick={async () => {
                if (
                  confirm('¿Estás seguro que deseas eliminar este cliente?')
                ) {
                  //! change to toast
